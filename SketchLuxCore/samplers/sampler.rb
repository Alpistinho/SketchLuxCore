module SketchLux
	...
	class Sampler
		...
		#This Hash holds the parameters shared between all samplers
		...
		@@typeParameters["SOBOL"] = []
		@@typeParameters["METROPOLIS"] = ["largesteprate", "maxconsecutivereject", "imagemutationrate"]
		...
		def definition()
			...
			definitionLines ||= []
			definitionLines << 'sampler.type = ' + @type
		...
		def parameterDefinition(parameterName)
			...
			if parameterString
				return 'sampler.' + @type + '.' + parameterName  + ' = ' + parameterString
			end
